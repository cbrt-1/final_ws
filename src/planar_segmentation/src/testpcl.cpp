...
int main() {
  ...
  for (const auto& filtered_cluster : filtered_clusters_by_points) { // Use the filtered list
  ...
  if (!combined_filtered_indices->indices.empty())
  {
    ...
    if (!cluster_indices.empty()) {
        ...
        for (const auto& indices : cluster_indices) {
            ...
            centroid_point.r = 255; centroid_point.g = 0; centroid_point.b = 0; // Example: Red centroids
